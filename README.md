...
SQLite is created in `HAILMAP_DATA_DIR`, or in `.data/` when that variable is unset. The bundled Census cache stays in `data/census/` and is not the database directory.
...
| `HAILMAP_WEBHOOK_SECRET` | Required in production for `/api/webhooks/spotter` and `/api/webhooks/community`. |
| `HAILMAP_USER_AGENT` | Contact string for the NWS API. |
| `CAPACITOR_SERVER_URL` | HTTPS origin loaded by the Android WebView. |

## HTTP API

- `GET /api/reports` — fused reports for the last 7 days
...
3. Set `HAILMAP_DATA_DIR=/data`. The image also sets this.
...
Vitest covers hail size parsing (including SPC hundredths), damage tags, dedupe/fusion, and swath clustering, hulls, and buffers.
